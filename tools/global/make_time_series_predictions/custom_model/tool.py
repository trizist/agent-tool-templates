...
from time_series_helpers import prepare_prediction_data
...
def make_datarobot_ts_predictions(
    deployment_id: str,
    ...
    Takes in either a Pandas Dataframe OR a JSON formatted Array and uses that to request a prediction from the specified DataRobot model.

    Note the argument `columns_to_return_with_predictions` this will tell the tool to return columns from the input dataset if you want. Use this to make sure you can interpret the predictions. For example, you may want to return the date and series columns so you can see the date of the prediction and which series the prediction is for.
    ...
    forecast_point: datetime
        the forecast point to use for time series predictions. (Required)

    columns_to_return_with_predictions: List[str]
        a list of columns from the input data that will merged back onto the ouput data for refence. (Required)
    ...
    input_dataframe: pd.DataFrame
        You may pass a pandas dataframe or a dataframe cache-id for predictions
    ...
    input_data_adjusted = get_adjusted_input_data(deployment, input_data, is_timeseries=True)
    expls_no = min(10, len(input_data_adjusted.columns))

    input_data_adjusted, _, _ = asyncio.run(prepare_prediction_data(rmv, input_data_adjusted))
    df, _ = predict(
        deployment,
        input_data_adjusted,
        max_explanations=expls_no,
        time_series_type=TimeSeriesType.FORECAST,
